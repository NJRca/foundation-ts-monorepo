'use strict';
/* eslint-disable no-sparse-arrays, no-process-env */
const __awaiter =
  (this && this.__awaiter) ||
      ...
      if (current.count > config.maxRequests) {
        res.status(429).json({
          ...
          message: 'Rate limit exceeded. Max '
            .concat(config.maxRequests, ' requests per ')
            .concat(config.windowMs, 'ms.'),
...
const RouteBuilder = /** @class */ (function () {
  ...
  RouteBuilder.prototype.middleware = function () {
    ...
    this.config.middleware = __spreadArray(
      __spreadArray([], this.config.middleware || [], true),
      middleware,
      true
    );
    return this;
