...
import { LogEntry, LogParser, SelfHealIngest } from '../src/index';
...
describe('SelfHealIngest', () => {
  let ingest: SelfHealIngest;

  beforeEach(() => {
    ingest = new SelfHealIngest(mockEventBus as any);
    jest.clearAllMocks();
  });
  ...
  describe('log processing', () => {
    ...
    it('should handle multiple similar errors', async () => {
      ...
      // Should group similar errors and potentially trigger healing
...
describe('LogParser', () => {
  ...
  describe('Container log parsing', () => {
    it('should parse container logs with JSON content', () => {
      ...
      expect(parsed?.service).toBe('api'); // Should use service from JSON
    });
